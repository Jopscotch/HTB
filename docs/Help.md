...
## Exploitation
Was able to login to machine using cracked user/pass. Can submit tickets. Found two possible routes to exploitation for HelpDeskZ v1.0.2 via `searchsploit`. 

The first involves submitting a ticket with a PHP reverse shell file. This was an interesting method to explore. You can submit a PHP file, but it tells you that such files are not allowed. Some internet guides for Help indicate that it still uploads, but that felt like a guess. Using Burp's interspector, I was able to submit the ticket with attachment, catch the submission, modify the content type to "image/jpeg" and then have it go through successfully. 

That was great and all, but we still need to be able to execute it. So, the question becomes, where the heck do the uploads hide and what are they named. In poking around in a copy of the HelpDeskZ v1.0.2 git, we can determine that the likely location of the uploads is `help.htb/support/tickets/uploads`. We also note from our searchsploit script and CVE that the filenames are coded based on upload time and hashed using md5. The searchsploit script used to hunt the filename down was a bit clunky. I rewrote it for python3 as shown below. Once the file was submitted with the ticket, I spun up a netcat instance for port 1234 and ran my script. This got me my shell and I quickly snagged the user flag. 
